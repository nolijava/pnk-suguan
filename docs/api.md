...
| Method | Path | Permission | Description |
|---|---|---|---|
| POST | `/api/scheduling/generate` | scheduling.generate | `{ weekId }` or `{ year, week }` (auto-creates DRAFT week), `mode`: `auto` (default) · `manual` · `destinado` · `katuwang`. **Update #22 — WEEKLY AVAILABILITY PREREQUISITE:** every mode is gated on the SELECTED week's availability; an incomplete week answers `422 AVAILABILITY_REQUIRED` (audited `GENERATION_BLOCKED`) and for `{ year, week }` the week row is not even created. `mode: "manual"` is gate-only — it validates and persists nothing, returning `{ weekId, year, week, manual: true }`. Generation returns `{ weekId, regenerated, plan, inserted }`; 409 outside DRAFT |
| GET | `/api/scheduling/generation-gate?weekId=` or `?year=&week=` (optional `&mode=` for the audit) | scheduling.generate | **Update #22** — READ-ONLY Weekly Availability probe used on Confirm before generation (never creates a week row): `{ ready, missing, total, year, week, weekId, message }`, or `422 AVAILABILITY_REQUIRED` carrying the blocking sentence when required (master-ACTIVE) teachers still lack a row for that ISO week |
...
### Filter parameters are forwarded explicitly (Update #24)

The availability PAGE forwards only `AVAILABILITY_FILTER_KEYS` to the strict `availabilityQuerySchema`. It also receives its own `year`/`week` (week navigation and the ISO-week jump form) and, after a redirect, `notice`/`error` — none of which the filter schema accepts. Because the page falls back to “no filters at all” when that strict parse fails, spreading the whole query string in **silently discarded every active filter**: the list looked unfiltered while the controls displayed the filter as applied. The JSON API (`GET /api/availability`) keeps the strict contract unchanged — a caller passing anything outside the set is still a caller error.
...
**Weekly Availability readiness per week column (Update #23).** Every ISO-week header in all three tables carries a readiness mark so an operator sees which weeks are ready and which generation would be BLOCKED before pressing Confirm: a **filled** dot = the week's required availability is complete, a **ring** (different shape, never colour alone) = one or more required teachers are still missing. Both states expose the exact count in their accessible name and tooltip, and the mark links to that week's `/availability?year=&week=`, so a blocked week is one click from being fixed. The readiness comes from `AvailabilityService.getAnnualAvailabilityReadiness(year)` — a READ-ONLY, batched (three query) pass that uses the **same** required set (master-ACTIVE teachers) and the **same** missing-row semantics as the generation gate, so the indicator can never disagree with what generation will do; it never creates a week row. A legend above the tables names both states.
...
| `/availability` | availability.read | Phase 3 weekly encoding: ISO week nav (prev/next/current + year/week jump, 52/53-safe), effective-status filters incl. NOT_ENCODED, inline status/reason grid, batched save with unsaved-change count, Fill Blanks confirm dialog, master-inactive rows locked with explanation, PUBLISHED lock banner + SUPER_ADMIN correction flow; **Update #24 — `?fix=1` “Fix availability” guide** (reached from a blocked week's matrix mark or a generation block notice): shows the generation gate's OWN blocking sentence, highlights every teacher still needing availability (amber accent + `needs availability` hint, plus how many the active filters hide), offers the one-click **Fill N as AVAILABLE** (still behind its confirm dialog) and a “Show only the missing” link, and disappears by itself once nothing is missing. It is opt-in — the plain week view is unchanged |
| `/` | assignments.read | Phase 5+6 Home Dashboard: year nav (‹ ›, Today, current-ISO-week indicator), **Generate Suguan button (top-right, ADMIN/SCHEDULER — ISO WEEK + Generation Method modal; Auto-generate routes to the existing Phase 4 engine flow on /schedule, Manual opens /schedule, Cancel does nothing; never generates directly)**, Annual Suguan Schedule FIRST as three stacked Dako × ISO-week tables (SUGO → RESERBA → RESERBA II; synced horizontal scroll, sticky Dako column, current-week highlight, **Update #23 per-week availability READINESS mark on every ISO-week header — filled dot = ready, ring = blocked, exact count in the accessible name/tooltip, links to /availability?year=&week=, with a legend**, DISABLED badge on historical disabled-dako rows, **Dako Name only — codes hidden**; assigned cells clickable for ADMIN/SCHEDULER opening the Modify/Clear/Cancel prompt; ABSENT / [UPDATED] accessible badges + persisted-data tooltips), then real-count Current Week summary + quick links |
